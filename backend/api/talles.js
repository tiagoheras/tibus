...
tallesRouter.put('/:talleId', (req, res, next) => {
    const talle = req.body.talle.talle,
        cantidad = req.body.talle.cantidad,
        itemSku = req.body.talle.itemSku;
    db.get('SELECT * FROM Catalogo WHERE Catalogo.sku = $itemSku', {
        $itemSku: itemSku
    }, (err, catalogoItem) => {
        if (err) {
        ...
        db.run('UPDATE Talles SET talle = $talle, cantidad = $cantidad, item_sku = $itemSku WHERE Talles.id = $talleId', {
            $talle: talle,
            $cantidad: cantidad,
            $itemSku: itemSku,
